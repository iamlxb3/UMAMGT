...
def _save_basic_result(data_analyze_df, texts, dataset, src, language):
    ...
    # save sample num
    data_analyze_df['value'].append(len(texts))
    data_analyze_df['type'].append('sample_N')
    data_analyze_df['dataset'].append(dataset)
    data_analyze_df['src'].append(src)
    data_analyze_df['language'].append(language)
...
def main():
    ...
    for dataset in datasets:
        ...
        # # N-gram distinct
        # # --------------------------------------------------------------------------------------------------------------
        # text_analyzer = TextAnalyzer(do_lower=True, language=language, load_spacy_model=False)
        ...
        #                              language=language,
        #                              load_spacy_model=True)
        # human_concreteness_dict = text_analyzer.analyse_concreteness(human_texts[:debug_N], language)
        # machine_concreteness_dict = text_analyzer.analyse_concreteness(machine_texts[:debug_N], language)
        #
    ...
    basic_analysis_save_path = '../result/static_data_analysis/basic.csv'
    data_analyze_df.to_csv(basic_analysis_save_path, index=False)
    print(f"Save result to {basic_analysis_save_path}")
